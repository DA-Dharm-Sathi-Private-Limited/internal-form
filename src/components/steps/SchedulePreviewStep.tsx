    ...
    try {
      ...
      for (const { sh, i } of delhiveryShipments) {
        ...
        const payload = {
          ...
          total_amount: Math.min(49999, Number((amount || grandTotal).toFixed(2))),
          cod_amount: sh.payment_mode === 'COD' ? Math.min(49999, Number(sh.cod_amount ?? Number((amount || grandTotal).toFixed(2)))) : 0,
