...
export default function ShadowfaxExportButton() {
    ...
    const handleDownload = async () => {
        ...
        setErrorMsg('');
        try {
            const data = await shadowfaxService.download(
            ...
            if (data.success && data.files) {
                 if (data.files.length === 0) {
                     setErrorMsg('No Shadowfax Scheduled orders found in this time range.');
                     setLoading(false);
                     return;
                 }
                    ...
                    <div className="bg-white dark:bg-[#16161f] rounded-2xl shadow-xl w-full max-w-md p-6 border border-gray-200 dark:border-[#2a2a38] animate-in slide-in-from-bottom-4 zoom-in-95 duration-200">
                        ...
                        <div className="space-y-4 mb-6 text-sm">
                            <p className="text-gray-600 dark:text-gray-400">
                                Select the time range to export scheduled Shadowfax orders. This will generate a separate CSV for each origin warehouse.
                            </p>
